...
public final class Gemma4AssistantDraftModel: Module, MTPDrafterModel {
    ...
    public func draftBlock(
        ...
        // top-level `Gemma4` / `Gemma4Unified` (same text backbone) are
        // accepted here. Stateless wrt target by construction (no ivars
        // retain anything derived from `target`).
        let textLanguageModel: Gemma4TextLanguageModel
        switch target {
        case let g4 as Gemma4: textLanguageModel = g4.languageModel
        case let g4u as Gemma4Unified: textLanguageModel = g4u.languageModel
        default:
            fatalError(
                "Gemma4AssistantDraftModel.draftBlock: target is not a Gemma 4 VLM "
                    + "(got \(type(of: target)))")
        }
        let backbone = textLanguageModel.model
