...
public final class Gemma4: Module, VLMModel, KVCacheDimensionProvider {
        ...
        -> PrepareResult
    {
        ...
        if let imagePixels = input.image?.pixels {
            ...
            // No chunked prefill on the image path: the blockwise
            // bidirectional vision overlay needs every image token of a
            // block in the same forward pass, which chunk boundaries can't
            // guarantee. Text-only prompts (below) still prefill in chunks.
